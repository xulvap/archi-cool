...
Carte interactive du patrimoine architectural français méconnu — mouvement moderne, brutalisme, Art Déco, architecture industrielle et rationaliste. 241 lieux protégés (Monuments Historiques), données officielles Mérimée/POP (Ministère de la Culture).
...
- `data.json` — les 241 lieux (nom, architecte, commune, statut de protection, style, genre, époque, historique sourcé, etc.), chargé au démarrage via `fetch('./data.json')`.
